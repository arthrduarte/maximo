...
export default async function createShortSummary(
  ...
  try {
    ...
    Create a short, brief summary that captures the main points of the conversation.
    ...
    const systemPrompt = "You are an executive coach reviewing your own coaching conversations to summarize them. Create a short and brief summary that captures the main points of the coaching conversation. Use natural language and a professional tone.";
    ...
    try {
      ...
      console.log('📝 Generated short summary with Anthropic');
      ...
      console.log('📝 Generated short summary with OpenAI fallback');
    }

    return result;
  } catch (error) {
    console.error('❌ Error creating short summary:', error instanceof Error ? error.message : 'Unknown error');
    return null;
  }
}
